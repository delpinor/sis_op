...
alloc_block(void)
{
	...
	for (int i = 0; i < super->s_nblocks; i++) {
		if (block_is_free(i)) {
			// Marcamos como 'en uso' al nuevo bloque.
			bitmap[i / 32] &= ~(1 << (i % 32));
...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	// Fuera de rango( mayor a 10 + 1024)
	if (filebno >= NDIRECT + NINDIRECT)
		return -E_INVAL;
	if (filebno < NDIRECT) {  // Caso bloques directos
		*ppdiskbno = &f->f_direct[filebno];
	} else {  // Caso bloques indirectos.
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	uint32_t r;
	uint32_t *disk_blockno;
	uint32_t new_blockno;
	if ((file_block_walk(f, filebno, &disk_blockno, true)) < 0) {
		return -E_INVAL;
	}
	if (!*disk_blockno) {
		if ((new_blockno = alloc_block()) < 0) {
