...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	// Se transforma el “file id” en un struct Openfile
	// En caso de error, se retorna ese valor ( < 0)
	if ((r = openfile_lookup(envid, req->req_fileid, &openFile)) < 0)
		return r;

	// En caso de error, se retorna ese valor ( < 0)
	if ((r = file_read(openFile->o_file,
	                   ret->ret_buf,
	                   req->req_n,
	...
	// Se devuelve la cantidad de bytes leídos
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// En caso de error, se retorna ese valor ( < 0)
	if ((r = file_write(openFile->o_file,
	                    req->req_buf,
	                    req->req_n,
	                    openFile->o_fd->fd_offset)) < 0)
		return r;
